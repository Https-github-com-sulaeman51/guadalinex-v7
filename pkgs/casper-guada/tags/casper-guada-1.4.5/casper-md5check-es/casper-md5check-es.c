...
void plymouth_disconnected(void *user_data, ply_boot_client_t *client) {
        printf("Desconectado de Plymouth\n");
        got_plymouth = 0;
...
void plymouth_progress(ply_boot_client_t *client, int progress) {
  ...
  if (got_plymouth) {
    asprintf(&s, "md5check:comprobando:%d", progress);
...
int main(int argc, char **argv) {
  ...
  while (fscanf(md5_file, "%as %a[^\n]", &checksum, &checkfile) == 2) {
    ...
    plymouth_text(client, "Comprobando %s", checkfile);
    ...
    if (strncmp(hex_output, checksum, strlen(hex_output)) == 0) {
      ...
      plymouth_failure(client, "%s: difiere", checkfile);
      failed++;
    }
    free(checksum);
    free(checkfile);
  ...
  fclose(md5_file);
  if (failed) {
    plymouth_urgent(client, "Comprobacion finalizada: errores encontrados en %d archivos!", failed);
  } else {
    plymouth_urgent(client, "Comprobacion finalizada: no se encontraron errores.");
  }

  plymouth_prompt(client, "Pulse una tecla para reiniciar");
