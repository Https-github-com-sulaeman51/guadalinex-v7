	...
	$.fn.carousel = function(params){
		...
		var params = $.extend({
			...
			nextBtn: "<span>Siguiente</span>",
			prevBtn: "<span>Anterior</span>",
			btnsPosition: "inside",
	...
	function initPagination(env){
		...
		env.$elts.content.find("li").each(function(i){
			if (i % env.params.dispItems == 0) {
				env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<a class="paginacion-slider"><span>'+( env.$elts.paginationBtns.length + 1 )+'</span></a>').data("firstStep", i) );
			}
		});
